...
def generate_noise(size,num_samp=1,device='cuda',type='gaussian', scale=1):
    """(size,num_samp=1,device='cuda',type='gaussian', scale=1)
...
def upsampling(im,sx,sy):
    ...
    双线性插值上采样
    """
    m = nn.functional.interpolate(im,size=[round(sx),round(sy)],mode='bilinear',align_corners=True)
    return m
...
def calc_gradient_penalty(netD, real_data, fake_data, LAMBDA, device):      # 计算梯度惩罚gradient punishment
...
def read_image(opt):
    x = img.imread('%s/%s' % (opt.input_dir,opt.input_name))    # Return a gray-image is MxN, an  RGB-image MxNx3 and an RGBA-image MxNx4. 
    x = np2torch(x,opt)
    x = x[:,0:3,:,:]
    return x
...
def np2torch(x,opt):
    ...
    """    
    if opt.nc_im == 3:
        x = x[:,:,:,None]    # x=[M,N,3,None]
        x = x.transpose((3, 2, 0, 1))/255      # x = [None ,3 ,M , N] / 255    == >   [N , C , H , W]/255
    else:
        x = color.rgb2gray(x)
        x = x[:,:,None,None]    # x = [H , W , None , None]
        x = x.transpose(3, 2, 0, 1)    # x = [None , None , H , W]
...
def adjust_scales2image(real_,opt):
    ...
    opt.num_scales = math.ceil((math.log(math.pow(opt.min_size / (min(real_.shape[2], real_.shape[3])), 1), opt.scale_factor_init))) + 1    # ceil()向上取整
    ...
    scale2stop = math.ceil(math.log(min([opt.max_size, max([real_.shape[2], real_.shape[3]])]) / max([real_.shape[2], real_.shape[3]]),opt.scale_factor_init))
    opt.stop_scale = opt.num_scales - scale2stop 
    print("opt.stop_scale:",opt.stop_scale )
    return real
...
def adjust_scales2image_SR(real_,opt):
    ...
    print("num_scale",opt.num_scales)
